...
import { readRegister, readRegisterAndProvideASCII, writeBit } from './modbus.js';
...
export async function handleFirstScan(io, part) {
  ...
  if (firstScanData === 'NG') {
    try {
      ...
      emitErrorEvent(io, 'FIRST_SCAN_ERROR', `Error during first scan: ${error.message}`);
      logger.error('Error during first scan:', error);
    }
  } else {
    logger.info("Machine won't run further since the component is already marked.");
    io.emit('machine-stop', {
      message: "Machine won't run further since the component is already marked.",
    });
...
export async function handleSecondScan(io, part) {
  try {
    ...
    emitErrorEvent(io, 'SECOND_SCAN_ERROR', `Error during second scan: ${error.message}`);
