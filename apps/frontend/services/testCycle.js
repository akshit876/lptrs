...
async function compareScannerDataWithCode(scannerData) {
  try {
    ...
    logger.error(`Error comparing scanner data with code file: ${error.message}`);
    throw error;
  }
}
...
export async function runContinuousScan(io = null, comService) {
  ...
  try {
    ...
    logger.debug('Waiting 5 seconds before retrying serial port initialization');
    await sleep(5000);
    // continue;
  }
  ...
  while (true) {
    ...
    try {
      ...
      try {
        ...
        scannerData = await new Promise((resolve, reject) => {
          ...
          writeBitsWithRest(1415, 0, 1, 100, false)
            ...
            .catch((err) => logger.error(`Error triggering scanner: ${err.message}`));
        });
      ...
      if (await checkResetOrBit(1410, 2, 1)) {
        logger.info('Reset detected while waiting for 1410.2, restarting cycle');
        continue;
      }
      logger.info('Clearing buffer before second scan...');
      comService.clearBuffer();
      ...
      logger.info(`Writing bit 1414.${secondScannerData !== 'NG' ? 6 : 7} to signal scan result`);
      await writeBitsWithRest(1414, secondScannerData !== 'NG' ? 6 : 7, 1, 200, false);
      logger.info(secondScannerData !== 'NG' ? 'Second scan OK' : 'Second scan NG');

      logger.info('Comparing scanner data with code');
      const isDataMatching = await compareScannerDataWithCode(secondScannerData);
      ...
      logger.info(`Writing bit 1414.${isDataMatching ? 3 : 4} to signal data match result`);
...
async function resetSpecificBits(register, bitsToReset) {
  try {
    logger.info(`Attempting to reset bits ${bitsToReset.join(', ')} in register ${register}`);
    ...
    logger.info(`Successfully reset bits ${bitsToReset.join(', ')} in register ${register}`);
  } catch (error) {
    if (error.message.startsWith('Timeout resetting bits')) {
      logger.error(`Operation timed out while resetting bits in register ${register}`);
    } else {
...
async function testBitManipulation() {
  try {
    ...
    logger.info(`Register value after reset: 0x${afterResetValue.toString(16)}`);
    ...
    if (afterResetValue === expectedValue) {
      logger.info('Reset operation successful: alternate bits were correctly reset');
    } else {
      logger.error(
